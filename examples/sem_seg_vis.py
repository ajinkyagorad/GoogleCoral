...
class SciFiPlotter(QtWidgets.QWidget):
    def __init__(self, model_path):
        ...
        self.cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # Use DirectShow as the backend
    ...
    def paintEvent(self, event):
        ...
        self.draw_semantic_segmentation(painter, x=SCREEN_WIDTH - 400, y=SCREEN_HEIGHT - 250, width=300, height=200)
        self.draw_camera_feed(painter, x=50, y=SCREEN_HEIGHT - 250, width=300, height=200)  # Display live camera feed
    ...
    def draw_semantic_segmentation(self, painter, x, y, width, height):
        # Original code commented out for debugging purposes
        ...
        # Resize the image and set input to interpreter
        ...
        # Instead of using real semantic segmentation, generate a dummy random result
        dummy_result = np.random.randint(0, 256, (self.height, self.width), dtype=np.uint8)
        mask_img = Image.fromarray(self.label_to_color_image(dummy_result).astype(np.uint8))

        # Convert to QImage and draw
        mask_img = mask_img.convert("RGBA")  # Ensure the image is in RGBA format
...
if __name__ == '__main__':
    ...
    sci_fi_plotter = SciFiPlotter(args.model)
    sci_fi_plotter.show()
    sys.exit(app.exec_())
